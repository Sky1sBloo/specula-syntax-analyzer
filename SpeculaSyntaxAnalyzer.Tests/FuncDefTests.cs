...
public class FuncDefTests
{
    ...
    public void BaseFunction()
    {
        ...
        if (node is BodyNode bodyNode)
        {
            ...
            Assert.That(funcDef.Parameters.Count, Is.EqualTo(0));
            Assert.That(funcDef.ReturnType.DataType.DataType, Is.EqualTo(DataTypes.VOID));
    ...
    public void FullDefinition()
    {
        ...
        if (node is BodyNode bodyNode)
        {
            ...
            Assert.That(funcDef.Parameters[0].Identifier, Is.EqualTo("x"));
            Assert.That(funcDef.ReturnType.DataType.DataType, Is.EqualTo(DataTypes.STRING));
            Assert.That(funcDef.ReturnType.Capabilities.capabilityList.Any(c => c.type == CapabilityTypes.THR_LOCAL), Is.True);
            Assert.That(funcDef.ReturnType.Capabilities.capabilityList.Any(c => c.type == CapabilityTypes.MUT), Is.True);
            Assert.That(funcDef.isAsync, Is.False);
        }
        Assert.That(analyzer.ErrorHandler.ErrorList.Count, Is.EqualTo(0));
    }
    ...
    public void ParamNoReturnType()
    {
        ...
        if (node is BodyNode bodyNode)
        {
            ...
            Assert.That(funcDef.Parameters.Count, Is.EqualTo(1));
            Assert.That(funcDef.Parameters[0].Identifier, Is.EqualTo("data"));
            Assert.That(funcDef.ReturnType.DataType.DataType, Is.EqualTo(DataTypes.VOID));
            Assert.That(funcDef.isAsync, Is.False);
        }
        Assert.That(analyzer.ErrorHandler.ErrorList.Count, Is.EqualTo(0));
    ...
    public void DefaultCapabilities()
    {
        ...
        if (node is BodyNode bodyNode)
        {
            ...
            Assert.That(funcDef.Parameters.Count, Is.EqualTo(2));
            Assert.That(funcDef.Parameters[0].Identifier, Is.EqualTo("a"));
            Assert.That(funcDef.Parameters[1].Identifier, Is.EqualTo("b"));
            Assert.That(funcDef.ReturnType.DataType.DataType, Is.EqualTo(DataTypes.INT));
            Assert.That(funcDef.isAsync, Is.False);
        }
        Assert.That(analyzer.ErrorHandler.ErrorList.Count, Is.EqualTo(0));
    }
    ...
    public void AsyncFunctionFullDefinition()
    {
        ...
        if (node is BodyNode bodyNode)
        {
            ...
            Assert.That(funcDef.Parameters.Count, Is.EqualTo(1));
            Assert.That(funcDef.Parameters[0].Identifier, Is.EqualTo("url"));
            Assert.That(funcDef.ReturnType.DataType.DataType, Is.EqualTo(DataTypes.STRING));
            Assert.That(funcDef.ReturnType.Capabilities.capabilityList.Any(c => c.type == CapabilityTypes.THR_LOCAL), Is.True);
    ...
    public void AsyncMultiParam()
    {
        ...
        if (node is BodyNode bodyNode)
        {
            ...
            Assert.That(funcDef.Parameters.Count, Is.EqualTo(2));
            Assert.That(funcDef.Parameters[0].Identifier, Is.EqualTo("input"));
            Assert.That(funcDef.Parameters[1].Identifier, Is.EqualTo("config"));
            Assert.That(funcDef.ReturnType.DataType.DataType, Is.EqualTo(DataTypes.INT));
            Assert.That(funcDef.ReturnType.Capabilities.capabilityList.Any(c => c.type == CapabilityTypes.THR_LOCAL), Is.True);
            Assert.That(funcDef.ReturnType.Capabilities.capabilityList.Any(c => c.type == CapabilityTypes.MUT), Is.True);
            Assert.That(funcDef.isAsync, Is.True);
